...
const Electronic = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get('https://zenreserve-lc.onrender.com/api/business/electronicservices');
        setData(res.data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const filteredData = data.filter(d =>
    ...
    (!category || d.category === category || d.listingType === 'Electronic Service') &&
    d.orgName?.toLowerCase().includes(search.toLowerCase())
  );

  const handleCardClick = (item) => {
    navigate('/electronic-login', { state: { electronic: item } });
  };

  const handleGoBack = () => navigate(-1);

  return (
    <Box sx={{ background: 'linear-gradient(135deg, #e3f2fd, #ffffff)', minHeight: '100vh', pb: 6 }}>
      {/* Header */}
      <Box sx={{
        ...
        <Typography variant="h4" fontWeight="bold" textAlign="center" sx={{
          ...
          color: '#1976d2', flexGrow: 1, mr: { xs: 0, sm: 2 }
        }}>
          {/* <ElectricalServicesIcon sx={{ mr: 1 }} /> */}
          {`Electronic Service Centers in ${area}`}
        </Typography>
      </Box>
